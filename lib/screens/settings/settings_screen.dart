    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text('Sign Out', style: GoogleFonts.plusJakartaSans(color: c.rose, fontWeight: FontWeight.w600))),
        ],
      ),
    );
    if (confirm == true) {
        ...
        children: [
          SafeArea(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  // ── PROFILE HERO — gradient background with avatar overlap ──
                  Container(
                    ...
                    decoration: const BoxDecoration(
                      gradient: LinearGradient(
                        ...
                        colors: [Color(0xFF2A1810), Color(0xFF1E1420), Color(0xFF0D0B0E)],
                      ),
                    ),
                    child: Stack(
                      ...
                      children: [
                        ...
                        Positioned(top: -30, left: 0, right: 0, child: Center(child: Container(
                          ...
                          decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [c.primary.withValues(alpha: 0.1), Colors.transparent])),
                        ))),
                        Column(children: [
                          // Avatar
                          Stack(
                            ...
                            children: [
                              Container(
                                ...
                                child: _profileImagePath != null
                                    ...
                                    : Text(_userInitial, style: GoogleFonts.outfit(fontSize: 32, fontWeight: FontWeight.w800, color: c.primary)),
                              ),
                              // Ring
                              Positioned(top: -5, left: -5, right: -5, bottom: -5, child: Container(
                                decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), border: Border.all(color: c.primary.withValues(alpha: 0.3), width: 1.5)),
                              )),
                              // Edit icon (proper icon, not emoji)
                              Positioned(top: -4, right: -4, child: GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Icon(ZussIcons.edit, size: 12, color: c.textSecondary),
                                ),
                              )),
                              // Pro badge
                              Positioned(bottom: -4, right: -4, child: Container(
                                ...
                                child: Row(mainAxisSize: MainAxisSize.min, children: [
                                  Icon(ZussIcons.verified, size: 10, color: Colors.white),
                                  const SizedBox(width: 2),
                                  Text('Pro', style: GoogleFonts.plusJakartaSans(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.white)),
                                ]),
                              )),
                            ],
                          ).zussHero(delay: 0),
                          ...
                          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                            Icon(ZussIcons.location, size: 13, color: c.textSecondary),
                            const SizedBox(width: 3),
                            Text('India${_userCity.isNotEmpty ? ' · $_userCity' : ''} · Member since 2025', style: GoogleFonts.plusJakartaSans(fontSize: 13, color: c.textSecondary)),
                          ]),
                  ...
                  ).zussEntrance(index: 0),

                  // ── STATS ROW — overlaps hero by -14px, frosted card ──
                  Transform.translate(
                    offset: const Offset(0, -14),
                    child: Container(
                      margin: const EdgeInsets.symmetric(horizontal: 24),
                      decoration: ZussGoTheme.frostedCardDecoration(context),
                      child: Row(children: [
                  ...
                  // ── TRUST SCORE — accent card ──
                  Container(
                    margin: const EdgeInsets.symmetric(horizontal: 24),
                    padding: const EdgeInsets.all(16),
                    decoration: ZussGoTheme.accentCardDecoration(context, c.sage),
                    child: Row(children: [
                      Container(
                        width: 52, height: 52,
                        decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: c.sage.withValues(alpha: 0.3), width: 4)),
                        alignment: Alignment.center,
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          _TrustBadgeChip(icon: ZussIcons.verified, label: 'Verified', color: c.sage, bgColor: c.sageSoft),
                          const SizedBox(width: 4),
                          _TrustBadgeChip(icon: ZussIcons.id, label: 'ID', color: c.lavender, bgColor: c.lavenderSoft),
                          const SizedBox(width: 4),
                          _TrustBadgeChip(icon: ZussIcons.trips, label: '$_tripCount trips', color: c.gold, bgColor: c.goldSoft),
                        ]),
                      ])),
                    ]),
                  ...
                  if (_userBio.isNotEmpty)
                    ...
                    ).zussEntrance(index: 2, baseDelay: 400),
                  ...
                  ).zussEntrance(index: 3, baseDelay: 450),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 24),
                    child: Column(children: [
                      _MenuItem(icon: ZussIcons.star, label: 'Trek Points', value: '0', bgColor: c.goldSoft, iconColor: c.gold, onTap: () => context.push('/matches')).zussEntrance(index: 0, baseDelay: 500),
                      const SizedBox(height: 6),
                      _MenuItem(icon: ZussIcons.trips, label: 'My Trips', value: '$_tripCount trips', bgColor: c.primarySoft, iconColor: c.primary, onTap: () => context.push('/trips')).zussEntrance(index: 1, baseDelay: 500),
                      const SizedBox(height: 6),
                      _MenuItem(icon: ZussIcons.sos, label: 'Emergency SOS', value: '0 contacts', bgColor: c.roseSoft, iconColor: c.rose, onTap: () => context.push('/settings/safety')).zussEntrance(index: 2, baseDelay: 500),
                      const SizedBox(height: 6),
                      _MenuItem(icon: ZussIcons.settings, label: 'Settings & Privacy', bgColor: const Color(0x08FFFFFF), iconColor: c.muted, onTap: () => context.push('/settings/edit-profile')).zussEntrance(index: 3, baseDelay: 500),
                      const SizedBox(height: 14),
                      // Log out
                      GestureDetector(
                        onTap: _handleLogout,
                        child: Container(
                          ...
                          child: Row(children: [
                            ZussIcon(ZussIcons.logout, size: 17, color: c.rose, bgColor: c.roseSoft, bgSize: 36),
                            const SizedBox(width: 14),
                      ...
                      ).zussEntrance(index: 4, baseDelay: 500),
                    ]),
                  ),
          ...
          Positioned(bottom: 0, left: 0, right: 0, child: const ZussGoBottomNav(currentIndex: 4)),
        ],
      ),
    );
  }
...
class _TrustBadgeChip extends StatelessWidget {
  ...
  const _TrustBadgeChip({required this.icon, required this.label, required this.color, required this.bgColor});
  @override
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 3),
      Text(label, style: GoogleFonts.plusJakartaSans(fontSize: 9, fontWeight: FontWeight.w700, color: color)),
    ]),
  );
}
...
class _MenuItem extends StatelessWidget {
  ...
  const _MenuItem({required this.icon, required this.label, this.value, required this.bgColor, required this.iconColor, required this.onTap});
  @override
  Widget build(BuildContext context) {
    final c = context.colors;
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: c.card, borderRadius: BorderRadius.circular(16), border: Border.all(color: c.border)),
        child: Row(children: [
          ZussIcon(icon, size: 17, color: iconColor, bgColor: bgColor, bgSize: 36),
          const SizedBox(width: 14),
          Expanded(child: Text(label, style: GoogleFonts.plusJakartaSans(fontSize: 14, fontWeight: FontWeight.w600, color: c.text))),
          if (value != null) Padding(padding: const EdgeInsets.only(right: 4), child: Text(value!, style: GoogleFonts.plusJakartaSans(fontSize: 12, color: c.muted))),
          Icon(Icons.chevron_right_rounded, color: c.muted, size: 18),
        ]),
      ),
    );
