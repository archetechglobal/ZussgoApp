...
class _SeeAllTravelersScreenState extends State<SeeAllTravelersScreen> {
  ...
  Widget _buildChip(String label, bool selected, VoidCallback onTap) {
    return GestureDetector(onTap: onTap, child: Container(
      ...
      decoration: BoxDecoration(color: selected ? ZussGoTheme.green : ZussGoTheme.bgMuted, borderRadius: BorderRadius.circular(12)),
    ...
    return Scaffold(backgroundColor: ZussGoTheme.bgPrimary, body: SafeArea(child: Column(children: [
      ...
      Padding(padding: const EdgeInsets.fromLTRB(22, 10, 22, 0), child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('All Travelers', style: ZussGoTheme.displaySmall),
          Text('${_filtered.length} travelers found', style: ZussGoTheme.bodySmall),
        ])),
        GestureDetector(
          onTap: () => setState(() => _showFilters = !_showFilters),
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(color: hasFilters ? ZussGoTheme.green : ZussGoTheme.bgMuted, borderRadius: BorderRadius.circular(12)),
      ...
      if (_showFilters) Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Age Range', style: ZussGoTheme.labelBold.copyWith(fontSize: 12)),
            Text('${_ageRange.start.round()} — ${_ageRange.end.round()}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ZussGoTheme.green)),
          ]),
          RangeSlider(values: _ageRange, min: 18, max: 60, divisions: 42,
              activeColor: ZussGoTheme.green, inactiveColor: ZussGoTheme.borderDefault,
              onChanged: (v) => setState(() { _ageRange = v; _applyFilters(); })),

          // Gender
          Text('Gender', style: ZussGoTheme.labelBold.copyWith(fontSize: 12)),
          ...
          // Travel Style
          Text('Travel Style', style: ZussGoTheme.labelBold.copyWith(fontSize: 12)),
          const SizedBox(height: 6),
          ...
          // Mindset
          Text('Mindset', style: ZussGoTheme.labelBold.copyWith(fontSize: 12)),
          const SizedBox(height: 6),
          ...
          if (hasFilters) GestureDetector(onTap: _clearFilters, child: Center(child: Text('Clear all filters', style: TextStyle(fontSize: 12, color: ZussGoTheme.rose, fontWeight: FontWeight.w600)))),
        ]),
      ),
      ...
      if (hasFilters && !_showFilters) Padding(padding: const EdgeInsets.symmetric(horizontal: 22, vertical: 6),
          child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
            ...
            GestureDetector(onTap: _clearFilters, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(border: Border.all(color: ZussGoTheme.borderDefault), borderRadius: BorderRadius.circular(8)),
                child: const Text('✕ Clear', style: TextStyle(fontSize: 10, color: ZussGoTheme.textMuted)))),
          ]))),
      ...
      Expanded(child: _loading
          ? const Center(child: CircularProgressIndicator(strokeWidth: 2, color: ZussGoTheme.green))
          : _filtered.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Text('🔍', style: TextStyle(fontSize: 32)), const SizedBox(height: 8), Text('No travelers match', style: ZussGoTheme.displaySmall), const SizedBox(height: 4), Text('Try adjusting your filters', style: ZussGoTheme.bodySmall)]))
          ...
          child: Container(padding: const EdgeInsets.all(12), margin: const EdgeInsets.only(bottom: 8), decoration: ZussGoTheme.cardDecoration,
              child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text('${t['city'] ?? 'Explorer'} • ${t['gender'] ?? ''}', style: ZussGoTheme.bodySmall),
                  const SizedBox(height: 4),
                  Wrap(spacing: 4, children: [
                    if (t['travelStyle'] != null) Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: ZussGoTheme.greenLight, borderRadius: BorderRadius.circular(6)),
                        child: Text('🎒 ${t['travelStyle']}', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w500, color: ZussGoTheme.green))),
                    if (t['schedule'] != null) Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: ZussGoTheme.bgMuted, borderRadius: BorderRadius.circular(6)),
                        child: Text(t['schedule'] == 'Early Bird' ? '🌅 Early Bird' : '🦉 Night Owl', style: const TextStyle(fontSize: 9, color: ZussGoTheme.textMuted))),
                  ]),
                ])),
                const Icon(Icons.chevron_right_rounded, color: ZussGoTheme.textMuted, size: 18),
              ])),
        );
      })),
    ])));
  ...
  Widget _activeChip(String label) => Container(margin: const EdgeInsets.only(right: 4), padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
      decoration: BoxDecoration(color: ZussGoTheme.green, borderRadius: BorderRadius.circular(10)),
